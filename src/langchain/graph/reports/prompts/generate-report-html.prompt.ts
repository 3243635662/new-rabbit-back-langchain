...
趋势预测必须基于数据，不得编造指标。如果有 comparison 数据，优先使用对比数据做趋势判断；如果没有 comparison 数据，可以基于当前指标的绝对值和业务常识做有限的趋势展望，但要注明"因无对比数据，趋势预测仅供参考"。`
  ...
  return [
    ...
    '13. 不允许改写输入中的数值。',
    '14. 不允许编造输入中不存在的业务数据。',
    '15. 如果输入中有 rawChartJson 字段，先尝试用 JSON.parse 解析它。如果解析失败，将它当作包含 ECharts option 的文本，尽力从中提取图表配置。',
    '16. 图表配置来自解析 rawChartJson 后得到的 charts 数组，或 chartResult.charts 数组。使用其中的 echartsOption 渲染图表。',
    '17. 每个图表容器 id 必须和 chart 的 id 完全一致。',
    ...
    '   - 先解析 rawChartJson（如果存在），或使用 chartResult.charts',
    '   - 遍历解析后的图表数组',
    '   - 每个图表生成一个图表卡片',
    '   - 图表卡片包含标题、说明（如有 description)、图表容器',
    '   - 使用 echarts.init 和 setOption 渲染 echartsOption',
    '   - 如果 rawChartJson 无法解析且 chartResult.charts 为空，跳过图表分析区块',
    '',
    ...
    '图表渲染脚本要求：',
    '',
    '1. 在 DOMContentLoaded 事件中渲染所有图表。',
    '2. 如果 ECharts 加载失败（typeof echarts === "undefined"），在每个图表容器中显示"图表加载失败"。',
    '3. 如果某个图表渲染失败，在对应容器中显示"图表渲染失败"。',
    '4. 所有图表渲染完成后必须设置 window.__REPORT_CHARTS_RENDERED__ = true。',
    '5. 所有图表实例需要监听 window resize 事件并调用 resize()。',
    '',
    '打印和导出要求：',
    '',
