...
趋势预测必须基于数据，不得编造指标。如果有 comparison 数据，优先使用对比数据做趋势判断；如果没有 comparison 数据，可以基于当前指标的绝对值和业务常识做有限的趋势展望，但要注明"因无对比数据，趋势预测仅供参考"。`
  ...
  return [
    ...
    '13. 不允许改写输入中的数值。',
    '14. 不允许编造输入中不存在的业务数据。',
    '15. 图表必须基于输入的 chartResult.charts[].echartsOption 渲染。',
    '16. 每个图表容器 id 必须和 chartResult.charts[].id 完全一致。',
    '17. 必须在 HTML 中包含图表渲染脚本。',
    '18. 图表渲染完成后，必须在页面上设置：',
